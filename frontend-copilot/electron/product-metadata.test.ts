...
describe('CanDue product metadata', () => {
  ...
  it('uses the packaged application id and localized display name for desktop notifications', () => {
    ...
    expect(mainSource).toContain("const ELECTRON_NOTIFICATION_GROUP = 'assistant-runs'")
    ...
    expect(mainSource).toContain('const toastOpenTag = buildDesktopNotificationToastOpenTag(request)')
    expect(mainSource).toContain("return '<toast>'")
    expect(mainSource).toContain('return `<toast tag="${escapedTag}" group="${escapedGroup}">`')
