...
describe('activateManagedRuntimeVersion', () => {
  it('replaces an existing version directory and removes the backup after a successful activation', async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), 'candue-runtime-install-shared-success-'))
    ...
    await writeFixtureFile(path.join(versionDir, 'payload.txt'), 'old-runtime')
    await writeFixtureFile(path.join(stagedVersionDir, 'payload.txt'), 'new-runtime')
    ...
    await expect(readFile(path.join(versionDir, 'payload.txt'), 'utf8')).resolves.toBe('new-runtime')
    await expect(readFile(paths.activePointerFile, 'utf8')).resolves.toContain(`"activeVersion": "${version}"`)
  ...
  it('restores the previous version directory when staged activation fails mid-switch', async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), 'candue-runtime-install-shared-rollback-'))
    tempRoots.push(tempRoot)
    ...
    await rm(paths.rootDir, { recursive: true, force: true })
    await writeFixtureFile(path.join(versionDir, 'payload.txt'), 'known-good-runtime')

    await expect(activateManagedRuntimeVersion(paths, version, missingStagedVersionDir)).rejects.toThrow()

    await expect(readFile(path.join(versionDir, 'payload.txt'), 'utf8')).resolves.toBe('known-good-runtime')
  ...
  it('activates a staged version normally when no previous version directory exists', async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), 'candue-runtime-install-shared-empty-'))
    tempRoots.push(tempRoot)
    const paths = createManagedRuntimeFamilyPaths(tempRoot, 'node')
    ...
    await rm(paths.rootDir, { recursive: true, force: true })
    await writeFixtureFile(path.join(stagedVersionDir, 'payload.txt'), 'fresh-runtime')

    const activatedVersionDir = await activateManagedRuntimeVersion(paths, version, stagedVersionDir)

    expect(activatedVersionDir).toBe(versionDir)
    await expect(readFile(path.join(versionDir, 'payload.txt'), 'utf8')).resolves.toBe('fresh-runtime')
    await expect(readFile(paths.activePointerFile, 'utf8')).resolves.toContain(`"activeVersion": "${version}"`)
