...
def test_persisted_thread_detail_response_models_timeline_items_and_configuration_snapshot() -> (
    ...
    response = PersistedThreadDetailResponse.model_validate(
        {
            "ok": True,
    ...
    timeline_items = response.to_dict()["timelineItems"]
    assert timeline_items[0]["kind"] == "user_message"
    assert timeline_items[1]["toolCallId"] == "tool.weather-current:call-1"
    assert timeline_items[1]["phases"][0]["phase"] == "completed"
    assert timeline_items[2]["status"] == "completed"
...
def test_persisted_run_replay_response_rejects_extra_fields_in_structured_blocks() -> (
    ...
    with pytest.raises(ValidationError, match="extra"):
        PersistedRunReplayResponse.model_validate(
            {
                "ok": True,
