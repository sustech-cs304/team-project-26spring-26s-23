...
def fetch_selected_courses_with_credentials(
    ...
    *,
    ...
    try:
        ...
        if tis_client.context.role_code is None:
            ...
            logger.info(
                ...
                payload={
                    ...
                    "source": "homepage" if homepage.role_codes else "default-student-selected-course-role",
                },
            )

        page_url = urljoin(service_config.base_url, _DEFAULT_TIS_SELECTED_COURSES_PAGE_PATH)
        current_term_url = urljoin(service_config.base_url, _DEFAULT_TIS_SELECTED_COURSES_CURRENT_TERM_PATH)
        api_url = urljoin(service_config.base_url, _DEFAULT_TIS_SELECTED_COURSES_API_PATH)
        ...
        logger.info("▶ 请求 TIS 当前选课学期上下文", payload={"current_term_url": current_term_url})
        ...
        actual_semester = _parse_selected_course_semester_argument(semester, current_semester=current_semester)
        ...
        request_payload = _build_selected_courses_base_payload(
            ...
            current_academic_year=current_semester.academic_year if current_semester is not None else actual_semester.academic_year,
            current_term_code=current_semester.term_code if current_semester is not None else actual_semester.term_code,
            current_semester_id=current_semester.semester_id if current_semester is not None else actual_semester.semester_id,
        ...
        logger.info(
            "▶ 请求 TIS 已选课程明细接口",
            payload={"api_url": api_url, "semester_id": actual_semester.semester_id, "semester_source": semester_source},
        )
        ...
        courses = extract_selected_course_records_from_json(payload, semester=actual_semester)
        summary = build_selected_course_summary(payload, courses=courses, page_num=page_num, page_size=page_size)
        probes = [
            _build_tis_probe_result(page_response, probe_label="selected-courses-page"),
            _build_tis_probe_result(current_term_response, probe_label="selected-courses-current-term", request_payload=current_term_request_payload),
            _build_tis_probe_result(api_response, probe_label="selected-courses-api", request_payload=request_payload, record_count=len(courses)),
        ]
        logger.info(
            "✅ TIS 已选课程明细查询完成",
            payload={
                ...
                "current_semester": None if current_semester is None else current_semester.semester_id,
        ...
        stats = resolved_db_manager.sync_selected_courses(resolved_owner_key, actual_semester.semester_id, courses)
        persistence_summary = TISPersistenceSummary(
            ...
            metadata={"semester_id": actual_semester.semester_id, "course_count": len(courses)},
        )
