...
class TISClient:
    ...
    def __init__(
        self, *, config: TISServiceConfig | None = None, logger: TISLogger | None = None
    ) -> None:
    ...
    def login(
        ...
        if self.logger is not None:
            self.logger.info(
                ...
                payload={
                    ...
                    "requested_role_code": _clean_text(role_code) or None,
                },
            )
        ok = self.cas_client.login(
            normalized_username, normalized_password, self.config.entry_url
        )
        cookie_summary = _summarize_cookie_names(self.get_cookies())
        if self.logger is not None:
            self.logger.info(
                "ℹ TIS 登录后 Cookie 概览",
                payload={"entry_url": self.config.entry_url, **cookie_summary},
            )
        if not ok:
            if self.logger is not None:
                self.logger.error(
        ...
        verify_markers = _extract_response_auth_markers(
            verify_response, base_url=self.config.base_url
        )
        ...
        if self.logger is not None:
            verify_payload = {
                ...
                "warmup_step_count": len(
                    cast(list[dict[str, Any]], warmup_summary["steps"])
                ),
                "available_grade_menu_count": warmup_summary.get(
                    "available_grade_menu_count"
                ),
                **verify_markers,
                **cookie_summary,
            }
    ...
    def _warmup_authenticated_context(self) -> dict[str, Any]:
        ...
        def _capture_step(
            ...
            markers = _extract_response_auth_markers(
                response, base_url=self.config.base_url
            )
        ...
        _capture_step(
            "user-mk",
            self.context.post(
                urljoin(self.config.base_url, _DEFAULT_TIS_USER_MK_PATH),
                headers={"Accept": "*/*", "Referer": auth_main_url},
                label="TIS-User-MK",
            ),
        )
        _capture_step(
            "system-property",
            self.context.post(
                urljoin(self.config.base_url, _DEFAULT_TIS_SYSTEM_PROPERTY_PATH),
                headers={"Accept": "*/*", "Referer": dashboard_url},
                label="TIS-System-Property",
            ),
        )
        user_me_response, user_me_payload = _capture_step(
            "user-me",
            self.context.post(
                urljoin(self.config.base_url, _DEFAULT_TIS_USER_ME_PATH),
                headers={"Accept": "*/*", "Referer": dashboard_url},
                label="TIS-User-Me",
            ),
        )
        ...
        if derived_role_code and not self.context.role_code:
            ...
            if self.logger is not None:
                self.logger.info(
                    "ℹ 已从用户上下文补全 TIS RoleCode",
                    payload={
                        "resolved_role_code": self.context.role_code,
                        "source": "user.me",
                    },
                )
        queryxsxx_response, queryxsxx_payload = _capture_step(
            "queryxsxx",
            self.context.post(
                urljoin(self.config.base_url, _DEFAULT_TIS_QUERYXSXX_PATH),
                headers={"Accept": "*/*", "Referer": dashboard_url},
                label="TIS-QueryXsxx",
            ),
        )
        ...
        grade_modules = (
            get_mknode_payload.get("002")
            if isinstance(get_mknode_payload, dict)
            else None
        )
        available_grade_menu_count = (
            len(grade_modules) if isinstance(grade_modules, list) else None
        )
        authenticated = all(
        ...
        *,
        ...
        if normalized_method == "POST":
            response = self.context.post(
                ...
                json_data=json_data,
                headers=headers,
                label=f"TIS-Probe-{normalized_method}",
            )
        else:
            response = self.context.get(
                url,
