...
class FileToolGrepSearcher:
    ...
    def search(self, *, request: GrepRequest, resolution: PathResolution) -> GrepSearchPayload:
        ...
        candidates = list(_iter_matching_files(base_path=base_path, request=request, resolution=resolution))
    ...
    *,
    ...
    for index, line in enumerate(lines):
        ...
        if len(results) >= remaining:
            truncated = _line_has_following_match(lines=lines, start_index=index + 1, matcher=matcher)
            break
        before_buffer.append(line)
...
def _compile_file_glob(pattern: str) -> re.Pattern[str]:
    ...
    if normalized == "":
        raise FileToolError(code="invalid_pattern", message="fileGlob must be a non-empty string.")
    try:
...
def _compile_search_pattern(*, pattern: str, is_regex: bool, case_sensitive: bool) -> re.Pattern[str]:
