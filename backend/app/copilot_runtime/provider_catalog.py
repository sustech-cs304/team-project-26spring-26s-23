...
def list_provider_catalog_entries(*, runtime_status: ProviderRuntimeStatus | None = None) -> tuple[ProviderCatalogEntry, ...]:
    providers = load_provider_catalog().providers
    if runtime_status is None:
        return providers
    return tuple(provider for provider in providers if provider.runtime_status == runtime_status)
...
def get_provider_catalog_entry(provider_id: str) -> ProviderCatalogEntry | None:
    ...
    for provider in load_provider_catalog().providers:
        if provider.provider_id == normalized_provider_id or normalized_provider_id in provider.aliases:
            return provider
...
def _parse_provider_catalog(raw_registry: dict[str, Any]) -> ProviderCatalog:
    providers = tuple(_parse_provider_catalog_entry(item) for item in raw_registry["providers"])
    if len(providers) == 0:
        raise ValueError("Provider catalog must define at least one provider entry.")

    return ProviderCatalog(
        catalog_revision=_require_non_empty_string(raw_registry["catalogRevision"], label="catalogRevision"),
        providers=providers,
    )
...
def _parse_provider_catalog_entry(raw_entry: dict[str, Any]) -> ProviderCatalogEntry:
    return ProviderCatalogEntry(
        ...
        display_name=_require_non_empty_string(raw_entry["displayName"], label="displayName"),
        endpoint_type=_require_identifier(raw_entry["endpointType"], label="endpointType"),
        runtime_status=_require_runtime_status(raw_entry["runtimeStatus"], label="runtimeStatus"),
...
def _parse_auth_schema(raw_auth_schema: Any) -> ProviderCatalogAuthSchema:
    ...
    return ProviderCatalogAuthSchema(
        default_kind=_require_auth_kind(auth_schema["defaultKind"], label="authSchema.defaultKind"),
...
def _parse_model_config_policy(raw_model_config_policy: Any) -> ProviderCatalogModelConfigPolicy:
    model_config_policy = _require_mapping(raw_model_config_policy, label="modelConfigPolicy")
...
def _parse_capability_hints(raw_capability_hints: Any) -> ProviderCatalogCapabilityHints:
...
def _require_enum[TEnum: str](value: Any, *, allowed: tuple[TEnum, ...], label: str) -> TEnum:
    normalized = _require_non_empty_string(value, label=label)
