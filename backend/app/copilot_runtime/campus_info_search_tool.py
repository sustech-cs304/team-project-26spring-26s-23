...
async def execute_campus_info_search_tool(arguments: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    cache_dir = (
        Path(cache_dir_obj)
        if isinstance(cache_dir_obj, str) and cache_dir_obj.strip()
        else (BACKEND_DIR / "data" / "campus_docs")
    )
