...
class CanonicalThinkingCapability:
    ...
    def to_public_dict(self) -> dict[str, Any]:
        payload = {
            ...
            'defaultValue': None if self.default_value is None else self.default_value.to_dict(),
...
class ThinkingAdaptationResult:
    ...
    def to_public_dict(self) -> dict[str, Any]:
        payload: dict[str, Any] = {
            'requestedSelection': (
                None if self.requested_selection is None else self.requested_selection.to_dict()
            ),
            'appliedSelection': (
                None if self.applied_selection is None else self.applied_selection.to_dict()
            ),
...
def _build_openai_reasoning_effort(value: RuntimeThinkingValue) -> tuple[dict[str, Any], str] | None:
...
def _build_gemini_budget(value: RuntimeThinkingValue) -> tuple[dict[str, Any], str] | None:
    ...
    if value.mode == 'dynamic':
        return ({'extra_body': {'thinking': {'type': 'dynamic'}}}, 'gemini_budget_dynamic')
...
def _build_anthropic_budget(value: RuntimeThinkingValue) -> tuple[dict[str, Any], str] | None:
    if value.valueType != 'budget' or value.mode is None:
        return None
    if value.mode == 'off':
        return ({'thinking': {'type': 'off'}}, 'anthropic_budget_off')
    if value.mode == 'budget' and value.budgetTokens is not None:
        return ({'thinking': {'budget_tokens': value.budgetTokens}}, 'anthropic_budget_tokens')
    return None
...
def _build_qwen_switch(value: RuntimeThinkingValue) -> tuple[dict[str, Any], str] | None:
    if value.valueType != 'code' or value.code not in {'true', 'false'}:
        return None
    return ({'extra_body': {'enable_thinking': value.code == 'true'}}, f'qwen_switch_{value.code}')


def _build_fixed_reasoning(_value: RuntimeThinkingValue) -> tuple[dict[str, Any], str] | None:
...
def parse_thinking_capability_override(
    ...
    if supported not in {True, False}:
        return None
    source = _normalize_optional_string(raw_override.get('source')) or _DEFAULT_OVERRIDE_SOURCE
    visibility_record = raw_override.get('visibility') if isinstance(raw_override.get('visibility'), Mapping) else None
    visibility: dict[str, Any] | None = None
    ...
    editor_type = _parse_editor_type(
        template_record.get('editorType') if isinstance(template_record, Mapping) else None,
        fallback=None if base_spec is None else base_spec.editor_type,
    )
    allowed_values = _parse_allowed_values(
        template_record.get('allowedValues') if isinstance(template_record, Mapping) else None,
        fallback=() if base_spec is None else base_spec.allowed_values,
        editor_type=editor_type,
    )
    default_value = _parse_default_value(
        template_record.get('defaultValue') if isinstance(template_record, Mapping) else None,
...
def adapt_thinking_selection(
    *,
    ...
    if not _selection_value_allowed(normalized_requested_selection.value, capability.allowed_values):
        return _build_adaptation_result(
            requested_selection=normalized_requested_selection,
            applied_selection=None,
            applied=False,
...
def _resolve_verified_series_spec(model_route: ResolvedRuntimeModelRoute) -> _SeriesSpec | None:
    ...
    if provider != 'openai' or endpoint_type != 'openai-compatible':
        return None
    if model_id == 'gpt-5' or model_id.startswith('gpt-5-') or model_id.endswith('/gpt-5'):
...
def _resolve_override_series_spec(override_input: ThinkingCapabilityOverrideInput) -> _SeriesSpec | None:
    ...
    allowed_values = override_input.allowed_values if len(override_input.allowed_values) > 0 else (() if base_spec is None else base_spec.allowed_values)
    default_value = override_input.default_value if override_input.default_value is not None else (None if base_spec is None else base_spec.default_value)
    budget = override_input.budget if override_input.budget is not None else (None if base_spec is None else base_spec.budget)
...
def _normalize_allowed_values(
    *,
    ...
    if editor_type == 'fixed':
        fixed_values = tuple(value for value in candidate_values if value.valueType == 'fixed')
    ...
    code_values = tuple(value for value in candidate_values if value.valueType == 'code')
    return code_values
...
def _selection_value_allowed(
    ...
    if requested_value.valueType == 'budget' and requested_value.mode == 'budget' and requested_value.budgetTokens is not None:
        return True
    return any(_runtime_values_equal(requested_value, allowed) for allowed in allowed_values)


def _runtime_values_equal(left: RuntimeThinkingValue, right: RuntimeThinkingValue) -> bool:
...
def _normalize_runtime_selection(selection: RuntimeThinkingSelection) -> RuntimeThinkingSelection | None:
    ...
    if value.valueType == 'fixed':
        return RuntimeThinkingSelection(series=normalized_series, value=_fixed_value(value.labelZh or '固定推理'))
...
def _parse_default_value(
    ...
    *,
    ...
    if editor_type == 'fixed' and parsed.valueType != 'fixed':
        return fallback
    if editor_type == 'discrete' and not any(_runtime_values_equal(parsed, allowed) for allowed in allowed_values):
        return fallback
    return parsed
...
def _parse_budget_config(
    ...
    *,
    ...
    if min_tokens is None and max_tokens is None and step_tokens is None and not isinstance(anchor_tokens_raw, list):
        return fallback
...
def _parse_runtime_thinking_value(value: Any) -> RuntimeThinkingValue | None:
    ...
    if value_type == 'budget':
        ...
        if mode == 'budget' and budget_tokens is None:
            return None
        if mode == 'budget':
            return _budget_value('budget', label_zh or f'{budget_tokens} Tokens', budget_tokens)
        if mode == 'dynamic':
...
def _clamp_budget_tokens(value: int, config: ThinkingSeriesBudgetConfig) -> int:
    ...
    snapped = config.min_tokens + round((upper_bounded - config.min_tokens) / step) * step
