...
系统采用"Electron 桌面应用 + React 前端工作台 + Python 本地运行时"的混合桌面应用架构。整个系统并非传统的"B/S 架构网页客户端"，而是以本地优先原则构建的独立桌面数字生命助手。
...
系统由三个主要运行环境构成，这三部分各司其职，保证本地数据安全、桌面交互体验以及后端执行能力。

### 2.1 Electron 宿主（主进程）：安全边界与生命周期管理

Electron 主进程是整个系统的"大管家"与安全沙箱。它不直接渲染界面，也不负责 AI 对话生成，而是掌握用户最敏感的数据，并提供底层系统级能力。
...
- **安全隔离（宿主私桥）**：这是架构中最关键的安全设计。前端请求模型时只发送公开的"路由指纹"，真实的 API Key 仅保存在 Electron 中。Python 运行时在实际请求外部 LLM 之前，必须通过私有桥接通道向主进程发起请求，换取真实的鉴权信息。这保证敏感密钥既不在前端内存中暴露，也不会轻易随运行时的日志流出。
...
前端的设计重点在于：
- **启动态管理**：前端通过读取 Electron 暴露的公开配置快照以及 Python 运行时的就绪状态，决定应用是停留在启动屏、降级模式（仅有前端）还是正常进入主工作台。
- **聊天流消费**：进入聊天面板后，前端将作为 `run/stream` 流式事件的消费者。当后端返回 `run_started`、`tool_event`、`text_delta`、`run_completed` 时，前端依据这些事件实时更新界面。这种基于状态机的事件消费机制，取代了传统的一问一答阻塞请求。
...
1. **本地数据主权优先**：
   相较于传统的云端 SaaS 产品，架构将聊天历史（SQLite）、用户配置、各种凭证（Secrets）全部沉淀在用户本地物理设备上。Python 运行时仅作为本地服务工作，不存在集中的云端数据库收集用户聊天隐私。
2. **敏感信息的安全隔离**：
   如果将密钥下放到前端或直接写在后端的明文配置中，容易造成内存泄露或跨进程污染。"凭证托管在 Electron 主进程、执行时 Python 按需向主进程请求解析"模式，建立了一道坚固的安全隔离墙，确保 Python 运行时在"不知道密码库全貌"的情况下，仅能拿到当前请求合法授权的单次凭据。
3. **语言生态的优势互补**：
...
- **不依赖持续的云端同步**：架构预设历史记录恢复（Replay）仅在"单机本地 SQLite"语境下生效。应用重启时依赖本地数据库重建会话树，暂不提供跨设备的实时状态同步通道。
...
项目 CanDue 是一个桌面端智能助手工作台，因此 UI 设计重点放在最能体现项目特色的主要界面上，而不是登录、注册等通用页面。

设计遵循以下原则：

1. **任务导向**：用户应能快速完成"配置模型 — 选择模型 — 开始对话"的主流程。
2. **工作台式导航**：不同功能以工作区划分，降低认知负担。
3. **渐进展开**：高级配置在需要时再呈现，核心操作保持简洁。
4. **实时反馈**：聊天界面应清晰展示流式回复、工具执行状态与运行状态。
...
该界面用于应用启动时，向用户展示系统正在初始化与连接本地运行环境的状态。

**设计原因**：用户在启动应用时可能遇到多种情况：正常加载、本地服务连接中、启动失败。因此该页面不能只是一个通用 loading，而应明确说明系统当前处于哪个阶段。

![启动界面](media/image1.png)

**预期用户体验**：用户能够清楚了解应用是仍在加载中，还是已经真的失败。同时，用户可以直接在当前页面上执行恢复操作。

### 3.2 助手工作区

这是整个系统的核心界面，支持智能体选择、会话管理和 AI 对话。

**设计原因**：助手工作区是产品主路径。用户应当能够：
...
![助手工作区](media/image2.png)

**预期用户体验**：工作区应给人以专业、面向高效产出的感觉。三栏布局有助于用户随时保持对上下文的可见性。流式文本和工具执行状态能提升 AI 行为的透明度。

### 3.3 模型服务配置界面

该界面用于让用户配置不同的模型服务商资料，并管理可用模型列表。

**设计原因**：CanDue 并不假设系统默认已经具备可用模型服务，因此用户需要自行配置 API 服务商。该页面因此成为最重要的支撑型界面之一。

![模型服务配置界面](media/image3.png)

**预期用户体验**：用户应能够轻松地添加和管理多个服务商；API 密钥等敏感信息应在视觉上被隔离并得到安全处理。该页面应给人一种配置中心的感觉，而不是一个随意的表单页面。

### 3.4 默认模型设置界面

该界面用于定义系统默认使用的模型路由。

**设计原因**：用户不应该在每一个操作场景下都重复选择模型，因此系统需要提供默认模型设置，用于主助手流程和快速执行场景。

![默认模型设置界面](media/image4.png)

### 3.5 外部源界面

该界面用于导入或解析外部信息源，例如 WakeUP 分享链接。

**设计原因**：虽然该功能不如助手工作区那样核心，但它体现了项目的重要扩展方向，因此值得作为主要特色支持界面写入设计文档。

![外部源界面](media/image5.png)

### 3.6 Hub 风格的次级工作区

这些界面作为非核心但可扩展模块的结构化工作区，例如能力、文件、南科、日历工作区。

**设计原因**：这些模块各自已在不同程度上接入了真实数据面（能力工作区具备 MCP 管理、技能面板和工具权限；文件工作区具备文件树 CRUD 操作；日历工作区具备看板、Gantt 和 Timeline 视图），稳定的信息架构有助于持续扩展并保证整个应用结构一致。

下图为"能力工作区"的示例：

![能力工作区示例](media/image6.png)

## 4. 导航栏

整体导航采用**左侧边栏 + 右侧工作区内容区**的模式。
...
- **风格**：现代桌面生产力工具风格
- **颜色**：中性色深浅主题，重点操作使用强调色
- **字体**：清晰无衬线字体，保证可读性
- **组件**：卡片、列表、侧边栏、下拉框、弹窗
- **反馈**：对成功、运行中、警告、错误采用不同状态色
