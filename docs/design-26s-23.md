...
我们的项目 CanDue 是一个桌面端智能助手工作台，因此 UI 设计重点放在最能体现项目特色的主要界面上，而不是登录、注册等通用页面。
...
**预期用户体验**：用户能够清楚了解应用是仍在加载中，还是已经真的失败了。同时，用户可以直接在当前页面上执行恢复操作。
...
这些界面作为非核心但可扩展模块的结构化入口页，例如能力、文件、开发工作区。

**设计原因**：即使这些模块目前尚未像助手工作区那样成熟，稳定的信息架构仍有助于未来扩展，并保证整个应用结构一致。
