...
当 `app.isPackaged === false` 时，主进程会把工作区中的 [`backend/`](../../backend/) 作为后端源码目录，并通过 `python -m app.desktop_runtime` 启动本地 Python runtime。解析器会优先尝试工作区虚拟环境中的 Python，可用时直接使用该解释器。
