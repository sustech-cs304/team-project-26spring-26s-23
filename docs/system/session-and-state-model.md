---
title: 会话与状态模型
description: 解释公开配置中心、settings workspace、宿主运行态、前端 run 状态与后端 thread/run 存储当前怎样分层持有。
sidebar_position: 4
---
...
settings workspace 里仍然可以保留 `activeProviderId` 这一类页面交互状态，但它只适合作为设置页的编辑焦点、列表筛选或当前选中项。聊天区真正使用哪条模型路由，取决于每次 `run/start` 请求里显式携带的 `modelRoute`；兼容层 [`message/send`](./chat-runtime-contract.md) 也映射到同一语义，而不是某个全局 active provider。
...
前端当前会在 renderer 内存里维护 thread 壳与兼容会话壳。thread 壳至少会包含：
...
兼容会话壳仍会保留 `sessionId` 视图字段，并由同一底层 thread 投影出来。能力面字段会带出工具目录、推荐工具、工具选择模式和默认模型偏好提示。

### 6.3 当前默认发送路径不会自动启用推荐工具

虽然能力面仍然会返回推荐工具和默认偏好提示，但聊天草稿当前默认使用的是空工具数组。工具目录保留在能力面里，发送时是否携带工具，仍由当前消息请求显式决定。

### 6.4 后端正式历史保存在 `InMemorySessionStore`

当前 Python runtime 以 thread/run 为主模型持有记录。`InMemorySessionStore` 会保存 thread 记录、run 记录和 run 事件日志，并在需要时投影兼容会话视图。

多轮上下文的正式持有者，当前仍然是后端存储层，只是底层主键已经是 `threadId + runId`。
...
- thread 壳会在 `thread/create` 与 `thread/get` 完成后建立或刷新，兼容会话壳会在 `session/create` 与 `capabilities/get` 投影后刷新。
- 聊天面板里的 run 状态会随着 `run/stream` 事件流持续变化，兼容层 [`message/send`](./chat-runtime-contract.md) 也复用同一事件语义。
...
当前后端只在 `run_completed` 时把 assistant 文本写入正式历史。失败和取消都不会写入 assistant 成功消息。

此外，运行时已经收紧了一条诊断语义：当 raw collector 观察到 tool-call 参数完备却没有真实工具执行时，后端会显式发出诊断并以失败终止，不会静默完成。
