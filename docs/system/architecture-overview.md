...
- 它暴露 `/health`、`/ready`、`/version`、`/build-info`、`/diagnostics`、`/diagnostics/runtime-info` 以及 `/history/*` 这类控制面 / 历史端点。
- 它通过根路径 `POST /` 承载聊天相关方法，通过 history 路由承载持久化 thread list、detail、run replay 与 delete / purge / backup / restore。
- 它维护后端智能体目录、工具目录、SQLite chat session store 与 projection cache。
...
- 本地聊天历史已经切到 SQLite truth + projection 结构，重启后可以恢复 thread list、timeline 与基础 run replay。
...
- 当前历史恢复仍然是本地 SQLite 单机语境，不等于跨设备 / 云端同步。
