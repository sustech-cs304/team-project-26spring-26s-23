...
5. 后端以 SSE 事件流返回 `run_started`、`tool_event`、`text_delta` 和终态事件。
...
当前工具信息边界也已经稳定：工具目录由 `capabilities/get` 下发，请求层只通过 `enabledTools` 传启用工具 ID，真实工具生命周期通过 `tool_event` 在消息流里回传。
...
- `frontend-copilot/scripts/smoke-streaming-chat.mjs` 已经可以作为本地主线验收资产，并可在 `--enable-weather-tool` 模式下验证工具闭环。
