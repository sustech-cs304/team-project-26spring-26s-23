...
当前聊天持久化 schema 的 owner 是 `backend/app/copilot_runtime/persistence/` 与 `backend/alembic/`：

- ORM truth / projection 模型定义在 `backend/app/copilot_runtime/persistence/models/chat.py`
- 运行时默认会在 `SQLiteSessionStore` 初始化时执行 `alembic upgrade head`
- 任何 schema 变更都必须先修改 ORM，再通过 Alembic 生成和提交迁移
- `thread_projection` 与 `run_projection` 仍然是可重建 cache；不要把它们当成唯一真相来源
