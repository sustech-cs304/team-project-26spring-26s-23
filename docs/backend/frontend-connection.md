...
- 一组控制面端点。
- 一个统一的聊天根端点 `POST /`。
- 一组持久化历史与运维端点，用于读取历史线程、回放 run，以及执行 delete / purge / backup / restore。
...
- `DELETE /history/threads/{threadId}/purge`
...
- `delete` 只做软删除，默认列表会隐藏线程，但在 `purge` 前仍可读取明细；
- `purge` 会级联删除线程 truth、run、event 与 projection；
