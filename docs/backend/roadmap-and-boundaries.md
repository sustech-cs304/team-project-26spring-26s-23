---
title: 边界与未覆盖范围
description: 说明 backend 分册当前不应写成已实现事实的边界与未覆盖区域。
sidebar_position: 7
---

# 边界与未覆盖范围

这页只讲当前边界，不讲未来方案。需要讨论未来服务化方向时，请看[未来 API 草案参考](./reference-future-api-draft.md)。

## 当前边界先看一句话

今天的 `backend/` 已经具备本地桌面 runtime、session-first 聊天主契约、Blackboard 领域能力和 TIS 领域能力，但它还没有形成一套统一、稳定、面向前端的完整业务 Web API 服务。
