...
当前聊天 runtime 已经切到本地 SQLite 持久化。运行时或应用重启后：

- thread 列表可以从本地 truth / projection 恢复。
- 历史时间线可以按需重建并恢复。
- 已结束 run 可以从持久化事件做基础 replay。

但 backend 分册仍然不适合把当前聊天描述成“已经具备跨设备 / 云端历史管理能力”，因为它目前还不包括：
