...
先说结论：不要再把 backend 分册当成当前主入口。当前更适合先看 [给开发者](../developers/getting-started.md)、[后端实现](../developers/backend.md)、[聊天运行时](../developers/chat-runtime.md)、[Provider 与模型路由说明](../reference/providers-and-routing.md)、[运行时接口 / 事件参考](../reference/runtime-events.md) 和 [能力边界 / 状态总表](../reference/capabilities.md)。
...
3. Provider 相关事实不能再按“active provider 就是配置中心”的旧心智理解，统一口径请看 [Provider 与模型路由说明](../reference/providers-and-routing.md)。
...
| [边界与未覆盖范围](./roadmap-and-boundaries.md) | 补历史边界说明，不把它当成当前产品承诺。 |
