...
3. chat run 状态由聊天面板维护，描述当前这次 run 执行已经走到哪一步。
...
| `streaming` | assistant 占位项已经建立，前端正在持续接收 `tool_event` 与 `text_delta`。 |
...
- 收到 `tool_event` 时，状态保持 `streaming`，并更新同一 `toolCallId` 的工具步骤。
...
当流内出现“raw tool-call 参数完备但未发生真实工具执行”这一类诊断时，状态会先记录诊断摘要，再进入 `failed` 终态。
...
- 当前主链是 thread/run，`session/create`、`capabilities/get`、`message/send` 作为兼容壳继续保留。
