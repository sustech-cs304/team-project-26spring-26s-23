---
title: 系统架构
description: 说明 Electron 宿主、前端工作台、后端运行时、配置存储和共享目录之间的分工关系。
sidebar_position: 2
---

# 系统架构

最可靠的结构不是"一个前端连一个通用后端"，而是 **Electron 主进程负责宿主与配置边界，renderer 负责工作台与页面状态，Python runtime 负责控制面与 `thread/run` 执行链，`provider-catalog/` 负责共享目录事实**。
...
**Electron 主进程不是简单壳层，而是系统主边界的一部分。**
...
这也是为什么开发者文档必须把 Electron 宿主写成主链，而不是把旧兼容壳当成系统中心。

## 前端工作台负责什么

renderer 的重点不是文件 owner，而是**工作台和交互 owner**。最重要的两条 UI 主线是：

1. 助手工作区：进入聊天主链、消费流式事件、展示工具步骤和可见 Thinking 行为。
2. 设置工作区：维护模型服务、默认模型和其他设置项。

其余工作区已经有稳定结构，但很多数据面仍处在部分接通状态。判断边界时，回到[能力边界 / 状态总表](../reference/capabilities.md)。

## Python runtime 负责什么

Python runtime 是一个本地 loopback HTTP 服务。它负责：
...
如果你要看完整方法和事件集合，直接跳到[运行时接口 / 事件参考](../reference/runtime-events.md)。

## Provider 相关事实放在哪里

Provider 的共享目录事实不在前端页面代码，也不在 Python runtime 配置文件里单独各写一份。分工是：
...
配置 owner 在 Electron 主进程。Python runtime 解释启动参数，也在执行阶段按需取 route 解析结果，但不直接读取 settings workspace 文档。

### 前端页面结构尚未全部接通数据面

助手与设置已经是主线。能力、文件、开发等工作区已有结构、部分接通。
