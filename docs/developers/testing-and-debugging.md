---
title: 测试与调试
description: 汇总当前最常用的测试命令、smoke 入口和排查顺序，帮助开发者快速定位问题。
sidebar_position: 7
---

# 测试与调试

- 这页给谁看：准备验证修改、定位问题或做回归检查的开发者。
- 这页解决什么问题：把最常用的测试命令、smoke 脚本、文档构建命令和排查顺序放在一页里。
- 当前覆盖到哪：覆盖桌面前端、Python 后端和文档站点的常用验证入口；不展开所有历史测试资产细节。
- 当前状态：核心测试与 smoke 入口已可用；部分跨工作区或跨运行时的长期回归能力仍属部分接通。

先说结论：当前排查问题时，最有效的顺序通常是**先确认是哪一层出错，再跑对应层最小测试，最后再跑跨层 smoke 和文档构建**。
...
- 聊天合同或消息流问题：先跑 `frontend-copilot/src/features/copilot/` 附近测试。
- settings workspace 问题：先跑 `frontend-copilot/electron/settings-workspace/` 和 `src/workbench/settings/` 附近测试。
- 后端协议或 run 问题：先跑 `backend/tests/unit/` 下对应目录。

### 第三步：再用 smoke 确认跨层有没有断
...
如果你怀疑问题出在 text → tool → text 交错链、工具阶段、终态收口或后端 collector 选择，可以打开当前的调试环境变量：
...
这更适合在你已经确认基本链路能跑、但细节事件顺序或工具行为不对时使用。
