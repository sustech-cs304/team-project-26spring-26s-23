---
title: 配置与状态模型
description: 说明公开配置、settings workspace、secret 状态、宿主运行态、页面状态和后端存储之间的关系。
sidebar_position: 4
---

# 配置与状态模型

系统已经不是"一份 settings 文件 + 一份前端本地状态"的简单结构。**公开配置、settings workspace 普通状态、settings workspace secret 状态、宿主运行态、renderer 页面状态、后端 thread/run 存储各有 owner。**

## 状态分层表

| 状态层 | 主要 owner | 主要作用 | 状态 |
| --- | --- | --- | --- |
...
| 宿主运行态 | Electron 主进程 | 描述 Python runtime 是 starting、ready、failed 还是 degraded。 | 已可用 |
| renderer 页面状态 | 前端工作台 | 保存窗口的装配状态、run 状态、输入草稿和局部 UI 状态。 | 已可用 |
...
| 跨设备 / 云端历史同步 | 未形成稳定主线 | 尚未形成稳定主线。 | 规划中 |

## 公开配置快照

公开配置快照是"根装配输入"，不是"所有设置的真源"。

它主要服务这些场景：

- 启动时决定主题和基础外观。
- 帮 renderer 判断有没有可用的运行时地址。
- 提供少量适合公开投影的宿主信息。

如果你在改的是 provider profile、API key、默认模型，通常不应该先去找公开配置快照。

## settings workspace 为什么是配置主线

settings workspace 是设置工作区的主线。它至少分成两层：
...
**普通状态和 secret 状态都由 Electron 主进程持有。** Python runtime 不直接读取这些文档。

## Provider 相关状态常见注意点

### `activeProviderId` 不是聊天总开关

页面里可能仍有选中项、焦点项或列表上下文，但聊天真正走哪条模型路由，不由"切一个 active provider"单独决定。
...
默认模型路由会影响聊天草稿初始选择，但真正执行时，以本次请求显式携带的 route 为准。

## 宿主运行态怎样进入前端

Electron 主进程整理 hosted runtime 快照，再通过 preload 暴露给 renderer。对前端来说，这层状态回答：
...
这一层状态服务窗口内的体验尚未成为后端正式历史。

## 后端存储层保存什么

Python runtime 以 `thread/run` 为主模型保存：
...
这份历史可以跨 runtime / application 重启在本地恢复。但要注意：**恢复能力是单机本地 SQLite 语境。** 系统当前仅支持本地恢复，不具备跨设备同步、云端历史管理或全局 retention / search 能力。

## 配置和状态最常见的三条流转

### 1. 设置保存流

用户在设置页保存后，Electron 主进程分别写普通状态和 secret 状态，前端再重新加载或水合它们。
...
聊天发送时，前端从 settings workspace 取默认值或可选项，执行时再显式把 route、Thinking 和工具策略带进请求。
