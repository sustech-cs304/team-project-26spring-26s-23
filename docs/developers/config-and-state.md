...
| 后端 thread/run 存储 | Python runtime | 保存 thread、run、事件日志和正式归档结果。 | 已可用 |
| 跨 runtime 重启的完整历史恢复 | 当前未形成稳定主线 | 不能默认它已经存在。 | 规划中 |
...
当前要特别保守的一点是：**这份历史还不能默认跨 runtime 重启恢复。** 所以不要把当前系统写成已经具备成熟会话持久化能力。
...
| 后端会话存储 | `backend/app/copilot_runtime/session_store.py` |
